...
def fileReceive(connection, fileName): 
    try:
    ...
    while True:
        ...
        try:
            ...
            if not fileName.lower().endswith('.txt'):
                logging.warning(f"Invalid file extension received: {fileName}")
                connection.sendall(b"ERROR: Only .txt files are accepted")
                continue
            ...
            if fileReceive(connection, fileName):
                logging.info(f"successfully received file: {fileName}")
            else:
                logging.error("Failed to receive file")
